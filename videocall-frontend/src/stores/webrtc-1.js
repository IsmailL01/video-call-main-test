...
export const useWebRTCStore = defineStore("webrtc", () => {
  ...
  const initializeLocalMedia = async () => {
    try {
      ...
      const savedVideoState = storageService.getItem("videoEnabled", true);
      const savedAudioState = storageService.getItem("audioEnabled", true);

      if (videoTrack) {
        isVideoEnabled.value = savedVideoState;
        videoTrack.enabled = savedVideoState;
      }
      if (audioTrack) {
        isAudioEnabled.value = savedAudioState;
        audioTrack.enabled = savedAudioState;
      }
  ...
  const toggleVideo = () => {
    if (localStream.value) {
      ...
      if (videoTrack) {
        ...
        storageService.setItem("videoEnabled", isVideoEnabled.value);
  ...
  const toggleAudio = () => {
    if (localStream.value) {
      ...
      if (audioTrack) {
        ...
        // СОХРАНЕНИЕ СОСТОЯНИЯ В LOCALSTORAGE
        storageService.setItem("audioEnabled", isAudioEnabled.value);
